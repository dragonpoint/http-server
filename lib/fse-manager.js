...
class FseManager {
    constructor(options, logger) {
        ...
        this.options = {
            ...
            maxRetries: options.fse.maxRetries || 100,
            useMaxRetries: false,
    ...
    async start(httpServer) {
        ...
        this.logger.info('Restoring local state before first sync...');
        await this.scanLocalRecordsAndResume();

        if (this.p3dGetResultsUrl) {
            this.logger.info('Performing initial sync with P3D...');
            ...
            for (const p3dRecord of p3dRecords) {
                ...
                if (this.records.has(folderName)) {
                    ...
                    if (localRecord.State === eFSETestRecordState.eTRSOk) {
                        ...
                        // 2. 如果记录未下载完成 (状态为Downloading, FileError等)
                        //    则进行详细的文件信息对比和更新
                        const remoteRecordEquivalent = this.convertP3DRecordToLocal(p3dRecord, folderName);
                        let needsUpdate = false;

                        for (const fileTypeEnum in remoteRecordEquivalent.RecordFiles) {
                            const remoteFile = remoteRecordEquivalent.RecordFiles[fileTypeEnum];
                            const localFile = localRecord.RecordFiles[fileTypeEnum];

                            if (!localFile || localFile.Url !== remoteFile.Url || (localFile.Hash && remoteFile.Hash && localFile.Hash.toUpperCase() !== remoteFile.Hash.toUpperCase())) {
                                needsUpdate = true;
                                localRecord.RecordFiles[fileTypeEnum] = remoteFile;
                                localRecord.RecordFiles[fileTypeEnum].State = eFSETestFileState.eTFSMissing;
                            }
                        }

                        if (needsUpdate) {
                            this.logger.info(chalk.yellow(`  - Incomplete record ${folderName} has changed on server. Updating index file.`));
                            // 将更新后的信息写入 e9397340-a0ef-4c13-b3ac-d61d3298816f.json
                            await this.updateRecordFile(localRecord);
                            // 确保它仍在下载队列中以便处理新信息
                            this.enqueueDownload(localRecord);
                        }
                        
                        // 处理完这个未完成的记录后，跳到下一个P3D记录
                        continue;
                    }
                }
    ...
    async downloadFileWithRetry(record, fileType, attempt) {
        const fileInfo = record.RecordFiles[fileType];
        if (this.options.useMaxRetries && attempt >= this.options.maxRetries) {
        ...
        try {
            ...
            const expectedHash = fileInfo.Hash.toUpperCase();
            this.logger.info(chalk.blue(`    - MD5 Check for ${fileInfo.FileName}:`));
            this.logger.info(chalk.blue(`      - Calculated: [${calculatedHash}] (Length: ${calculatedHash.length})`));
            this.logger.info(chalk.blue(`      - Expected:   [${expectedHash}] (Length: ${expectedHash.length})`));
            ...
            // 如果文件名不是LOCAL_INDEX_FILENAME，则创建.MD5文件
            if (fileInfo.FileName !== LOCAL_INDEX_FILENAME) {
                const md5FilePath = localPath + '.MD5';
                await fs.writeFile(md5FilePath, fileInfo.Hash);
            }
            ...
            if (error.isAxiosError && error.response) {
                errorMessage = `Request failed with status code ${error.response.status} for URL: ${fileInfo.Url}`;
            } else if (error.isAxiosError) {
                errorMessage = `Request failed for URL: ${fileInfo.Url}. Message: ${error.message}`;
            }
            this.logger.info(chalk.red(`    - Attempt ${attempt + 1} for ${fileInfo.FileName} failed: ${errorMessage}`));
    ...
    async updateRecordInfoFromMetadata(record, metadataBuffer) {
        try {
            ...
            // Map fields from metadata.json to record.RecordInfo
            record.RecordInfo.ID = metadataContent.id;
            record.RecordInfo.TotalT = metadataContent['记录时长'];
            record.RecordInfo.MissionName = metadataContent['任务名'];
            record.RecordInfo.MissionID = metadataContent['任务ID'];
            record.RecordInfo.HardwareBaseID = metadataContent['底台ID'];
            record.RecordInfo.HardwarePanelID = metadataContent['面板ID'];
            record.RecordInfo.PlaneID = metadataContent['机型'];
            record.RecordInfo.TestDate = metadataContent['测试时间'];

            this.logger.info(chalk.blue(`  - Parsed metadata.json and updated record info for ${record.ParentFolderName}.`));

        } catch (e) {
            this.logger.info(chalk.red(`  - CRITICAL ERROR: Failed to parse metadata.json for ${record.ParentFolderName}. Error: ${e.message}`));
            ...
            if (p3dRecord[apiKey] && p3dRecord[apiKey].url) {
                ...
                try {
                    ...
                    record.RecordFiles[fileTypeEnum] = {
                        Version: 1, 
                        Url: cleanUrl, 
                        Hash: p3dFileInfo.hash ? p3dFileInfo.hash.trim() : p3dFileInfo.hash,
