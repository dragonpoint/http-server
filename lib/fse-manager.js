...
function getFolderNameFromUrl(urlString) {
    try {
        ...
        // [MODIFICATION] Use console.error for logging critical parsing failures
...
class FseManager {
    ...
    async downloadFileWithRetry(record, fileType, attempt) {
        ...
        try {
            ...
            // [MODIFICATION] Use this.logger.info with chalk.red instead of the non-existent this.logger.error
    ...
    convertP3DRecordToLocal(p3dRecord, folderName) {
        ...
        for (const apiKey in P3D_API_TO_FILE_TYPE_MAP) {
            if (p3dRecord[apiKey] && p3dRecord[apiKey].url) {
                ...
                try {
                    const cleanUrl = sanitizeUrl(p3dFileInfo.url);
                    const parsedUrl = new URL(cleanUrl);
                    const decodedPathname = decodeURIComponent(parsedUrl.pathname);
