...
// [MODIFICATION] START: Helper function to safely extract folder name from a URL
function getFolderNameFromUrl(urlString) {
    try {
        const parsedUrl = new URL(urlString);
        const pathParts = parsedUrl.pathname.split('/').filter(part => part.length > 0);
        if (pathParts.length >= 2) {
            // The folder name is the second to last part of the path (e.g., /<folderName>/metadata.json)
            // [FIX] Decode the URL component to handle non-ASCII characters
...
class FseManager {
        ...
        try {
            ...
            for (const entry of entries) {
                ...
                try {
                    ...
                    // [MODIFICATION] Use ParentFolderName as the key for the records map
    ...
    async syncWithP3D() {
        ...
        try {
            ...
            for (const p3dRecord of p3dRecords) {
                ...
                // [MODIFICATION] START: Use folder name from URL as the primary identifier
                const folderName = getFolderNameFromUrl(p3dRecord.metadata.url);
                if (!folderName) {
                    this.logger.info(chalk.red(`  - Could not determine folder name from URL for record id ${p3dRecord.id}. Skipping.`));
                    continue;
                }
                
                // Check for existence using the reliable folder name
                if (this.records.has(folderName)) {
                    ...
                    if (localRecord.State === eFSETestRecordState.eTRSOk) {
                        this.enqueueP3DDelete(localRecord.RecordInfo.ID); // Deletion still uses the internal ID
                    }
                    continue;
                }
    ...
    enqueueDownload(record) {
        // [MODIFICATION] Use ParentFolderName as the unique key for the download queue
    ...
    async downloadRecordFiles(record) {
        // [MODIFICATION] Base directories on the reliable ParentFolderName
        ...
        if (allFilesOk) {
            ...
            try {
                ...
                this.enqueueP3DDelete(record.RecordInfo.ID); // Deletion still uses the internal ID
            } catch (e) {
                this.logger.error(chalk.red(`Failed to finalize record in folder ${folderName}:`), e);
            }
        }
    }
    ...
    async downloadFileWithRetry(record, fileType, attempt) {
        ...
        try {
            ...
            // It's good practice to also save the MD5 hash for future local validation
    ...
    async updateRecordInfoFromMetadata(record, metadataBuffer) {
        try {
            ...
            // Only assign if it doesn't exist, to preserve the original ID from p3dRecord
            if (!record.RecordInfo.ID) {
                record.RecordInfo.ID = metadataContent.ID;
            }
            // You can choose to update other fields if necessary
            ...
            this.logger.error(chalk.red(`  - CRITICAL ERROR: Failed to parse metadata.json for ${record.ParentFolderName}.`));
    ...
    // [MODIFICATION] Function signature changed to accept folderName
    convertP3DRecordToLocal(p3dRecord, folderName) {
        const record = {
            Version: 1, 
            State: eFSETestRecordState.eTRSUnknown,
            RecordInfo: { ID: p3dRecord.id }, // Internal ID
            RecordFiles: {}, 
            ParentFolderName: folderName // The reliable, unique folder name
        };
    ...
    async updateRecordFile(record, isInDownloadingFolder=true) {
        const folderName = record.ParentFolderName;
        if (!folderName) {
            this.logger.error(chalk.red(`Cannot update record file for a record without a ParentFolderName.`));
            return;
        }
